...
from ..extractors import HTMLExtractor, ImageExtractor, PDFExtractor
...
class ScrapingPipeline:
    ...
    def __init__(self):
        ...
        if settings.openai_api_key:
            ...
            raise ValueError(
                "Se requiere configurar OPENAI_API_KEY o ANTHROPIC_API_KEY"
            )
            ...
            for evento in state["eventos_normalizados"]:
                if self._validate_event(evento):
                    ...
                    state["errores"].append(
                        f"Evento inválido: {evento.get('titulo', 'Sin título')}"
                    )
        ...
        try:
            ...
            if not evento.get("categoria") or not evento.get("descripcion"):
                ...
                try:
                    ...
                    if not evento.get("descripcion") and enhanced_data.get(
                        "descripcion"
                    ):
