...
class HTMLExtractor:
            ...
            try:
                ...
                if config.get("wait_for_selector"):
                    await page.wait_for_selector(
                        config["wait_for_selector"], timeout=settings.playwright_timeout
                    )
    ...
    async def _extract_single_event(self, container, campos: Dict, page: Page) -> Dict:
        ...
        for campo_nombre, campo_config in campos.items():
            try:
                valor = await self._extract_field_value(container, campo_config, page)
                ...
                if campo_config.get("requerido", False):
                    raise Exception(
                        f"Campo requerido '{campo_nombre}' no encontrado: {e}"
                    )
                elif campo_config.get("default"):
                    evento[campo_nombre] = campo_config["default"]
    ...
    async def _extract_field_value(
        self, container, campo_config: Dict, page: Page
    ) -> Optional[str]:
        """
    ...
    def _parse_date(
        self, date_str: str, formato: Optional[str] = None
    ) -> Optional[str]:
        """
