...
@tool
async def extract_pdf_direct(pdf_url: str, extraction_patterns: Dict[str, str]) -> Dict[str, Any]:
    """
    ...
    """
    tool_instance = BaseTool("extract_pdf_direct", "Extract events from single PDF document")
    ...
    try:
        ...
        return tool_instance._create_success_response(
            events,
            {"extraction_method": "pdf_direct", "events_found": len(events)}
        )
...
@tool
async def extract_multiple_pdfs(pdf_urls: List[str], extraction_patterns: Dict[str, str]) -> Dict[str, Any]:
    """
    ...
    """
    tool_instance = BaseTool("extract_multiple_pdfs", "Extract events from multiple PDF documents")
    
    try:
        tool_instance._log_execution("Starting multiple PDF extraction", f"PDFs: {len(pdf_urls)}")
        ...
        for i, pdf_url in enumerate(pdf_urls):
            try:
                tool_instance._log_execution("Processing PDF", f"{i+1}/{len(pdf_urls)}: {pdf_url}")
        ...
        tool_instance._log_execution("Multiple PDF extraction completed", 
        ...
        return tool_instance._create_error_response(f"Multiple PDF extraction failed: {str(e)}")


def _extract_events_from_text(content: str, patterns: Dict[str, str], source_url: str) -> List[Dict]:
