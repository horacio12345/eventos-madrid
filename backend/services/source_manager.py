...
class SourceManager:
    ...
    def create_source(self, source_data: Dict, created_by: str = "admin") -> FuenteWeb:
        ...
        existing = (
            self.db.query(FuenteWeb)
            .filter(FuenteWeb.nombre == source_data["nombre"])
            .first()
        )

        if existing:
            raise ValueError(
                f"Ya existe una fuente con el nombre '{source_data['nombre']}'"
            )
        ...
        fuente = FuenteWeb(
            ...
            frecuencia_actualizacion=source_data.get(
    ...
    def update_source_status(
        self, source_id: int, estado: str, error: str = None
    ) -> FuenteWeb:
        """
    ...
    def _validate_source_data(self, source_data: Dict, partial: bool = False) -> None:
        ...
        if "tipo" in source_data:
            ...
            if source_data["tipo"] not in valid_types:
                raise ValueError(
                    f"Tipo '{source_data['tipo']}' no válido. Debe ser uno de: {valid_types}"
                )
    ...
    def import_source_config(
        self, config_data: Dict, created_by: str = "admin"
    ) -> FuenteWeb:
        """
