...
from backend.core import get_db, get_settings, FuenteWeb
...
class ScrapingScheduler:
    ...
    def add_source_job(self, source_id: int, cron_expression: str = None) -> bool:
        ...
        try:
            with next(get_db()) as db:
                ...
                cron_expr = cron_expression or fuente.frecuencia_actualizacion or settings.default_update_frequency
                ...
                print(f"Trabajo programado para fuente '{fuente.nombre}' con cron: {cron_expr}")
                return True
    ...
    def update_source_schedule(self, source_id: int, new_cron: str) -> bool:
        ...
        try:
            ...
            with next(get_db()) as db:
                source_manager = SourceManager(db)
                source_manager.update_source(source_id, {
                    "frecuencia_actualizacion": new_cron
                })
    ...
    def get_scheduler_status(self) -> Dict:
        ...
        return {
            ...
            "scraping_jobs": len([j for j in self.scheduler.get_jobs() if j.id.startswith("scraping_fuente_")]),
            "next_runs": [
                {
                    "job_name": job.name,
                    "next_run": job.next_run_time.isoformat() if job.next_run_time else None
