...
class ScrapingPipelineState(TypedDict):
    ...
    overall_status: Literal["initializing", "scraping", "processing", "supervising", "completed", "failed"]
...
class StateBuilder:
    ...
    def with_source_info(self, url: str, source_type: str, schema: Dict, mapping: Dict, config: Dict):
    ...
    def build(self) -> ScrapingPipelineState:
        ...
        return ScrapingPipelineState(
            # Pipeline identification
            execution_id=self._state_data.get("execution_id", f"exec_{int(datetime.now().timestamp())}"),
...
def update_state_with_agent_result(
    ...
    if agent_name == "scraping_agent":
        ...
        state["validation_results"]["total_extracted"] = processing_metadata.get("total_raw", 0)
        state["validation_results"]["total_normalized"] = processing_metadata.get("normalized", 0)
        state["validation_results"]["total_validated"] = processing_metadata.get("validated", 0)
...
def finalize_pipeline_state(state: ScrapingPipelineState) -> ScrapingPipelineState:
    ...
    try:
        ...
        state["pipeline_metadata"]["total_duration_seconds"] = state["total_duration_seconds"]
    except ValueError:
    ...
    if validation["total_extracted"] > 0:
        validation["quality_score"] = (
            ...
            max(0, 1 - len(state["pipeline_errors"]) / 10)
        ) * 0.3
