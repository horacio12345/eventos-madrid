...
from backend.core.models import Configuracion, Evento, FuenteWeb, LogScraping
from backend.scraping.engine import ScrapingEngine
from backend.services.scheduler import (get_scheduler, start_scheduler,
                                        stop_scheduler)
...
class SystemManager:
    ...
    def status(self):
        ...
        try:
            ...
            eventos_activos = (
                self.db.query(Evento).filter(Evento.activo == True).count()
            )
            total_fuentes = self.db.query(FuenteWeb).count()
            fuentes_activas = (
                self.db.query(FuenteWeb).filter(FuenteWeb.activa == True).count()
            )
            ...
            # Últimos logs
            recent_logs = (
                self.db.query(LogScraping)
                .order_by(LogScraping.fecha_inicio.desc())
                .limit(5)
                .all()
            )
            ...
            if recent_logs:
                for log in recent_logs:
                    estado_emoji = (
                        ...
                        else "❌" if log.estado == "error" else "⏳"
                    )
                    fecha = log.fecha_inicio.strftime("%d/%m %H:%M")
                    print(
                        f"   {estado_emoji} {fecha} | {log.fuente_nombre} | {log.eventos_extraidos or 0} eventos"
                    )
            else:
            ...
            try:
                ...
                print(f"\n⏰ SCHEDULER:")
                print(
                    f"   Estado: {'🟢 ACTIVO' if scheduler_status['running'] else '🔴 INACTIVO'}"
                )
    ...
    def cleanup_old_data(self, days: int = 30):
        ...
        try:
            ...
            old_events = (
                ...
                .filter(
                    Evento.activo == False, Evento.ultima_actualizacion < cutoff_date
                )
                .count()
            )
            ...
            old_logs = (
                self.db.query(LogScraping)
                .filter(LogScraping.fecha_inicio < cutoff_date)
                .count()
            )
            ...
            try:
                ...
                for detalle in resultado["detalles"]:
                    ...
                    if detalle["estado"] == "success":
                        ...
                        print(
                            f"      Actualizados: {detalle.get('eventos_actualizados', 0)}"
                        )
                    else:
        ...
        try:
            ...
            for fuente in fuentes:
                ...
                if fuente.ultima_ejecucion:
                    print(
                        f"   Última ejecución: {fuente.ultima_ejecucion.strftime('%d/%m/%Y %H:%M')}"
                    )
                if fuente.eventos_encontrados_ultima_ejecucion:
                    print(
                        f"   Eventos última vez: {fuente.eventos_encontrados_ultima_ejecucion}"
                    )
                print()
