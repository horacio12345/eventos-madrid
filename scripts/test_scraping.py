...
async def test_source_scraping(source_id: int = None, source_name: str = None):
    ...
    try:
        ...
        if source_id:
            ...
            fuente = next(
                (f for f in fuentes if source_name.lower() in f.nombre.lower()), None
            )
            if not fuente:
                print(
                    f"❌ No se encontró fuente con nombre que contenga: {source_name}"
                )
                return
        else:
        ...
        if resultado["estado"] == "success":
            ...
            if resultado["preview_eventos"]:
                ...
                for i, evento in enumerate(resultado["preview_eventos"], 1):
                    ...
                    print(
                        f"      Ubicación: {evento.get('ubicacion', 'Sin ubicación')}"
                    )
        ...
        try:
            with open(filename, "w", encoding="utf-8") as f:
                json.dump(
                    resultado_completo, f, indent=2, ensure_ascii=False, default=str
                )
